...
void showPartAsRect(Mat img, MatPart part, string wn, DocumentPage* p) 
{
	if (p != NULL) {
		print_byte_vector(p->text(part));
		cout << " \n";
	}
...
void showParts(Mat img, MatPart whole, string wn, DocumentPage* p) 
{
	namedWindow(wn);
	if (whole.childs.empty()) 
	{
		showPartAsRect(img.clone(), whole, wn, p);
	}
	else 
	{
		for (int i = 0; i<whole.childs.size(); i++) 
		{
			showParts(img, whole.childs.at(i), wn, p); 
		}
	}
}
